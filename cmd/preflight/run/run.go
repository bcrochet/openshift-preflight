...
import (
	...
	"github.com/redhat-openshift-ecosystem/preflight/format"
...
func NewCommand() *cobra.Command {
	...
	for i, p := range preflight.RegisteredPlugins() {
		invocation := i
		plug := p
		config := spfviper.New()
		plcmd := preflight.NewPluginCommand(config, invocation, plug)
		plcmd.RunE = func(cmd *cobra.Command, args []string) error {
			return run(cmd, args, invocation, config, &preflight.ResultWriterFile{})
		}
...
func run(
	...
	resultWriter preflight.ResultWriter,
) error {
	ctx := cmd.Context()
	...
	// Run the plugin
	plugin := preflight.RegisteredPlugins()[pluginName]
...
var formatAsJSON format.FormatterFunc = func(ctx context.Context, r preflight.Results) (response []byte, formattingError error) {
	f, _ := format.NewByName("json")
