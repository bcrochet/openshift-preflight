...
// NewBackwardsCompatCheckCommand is a compatibility command bridging Preflight's
...
func containerPlugin(config *viper.Viper) *cobra.Command {
	...
	plug := preflight.RegisteredPlugins()["check-container"]
	plcmd := preflight.NewPluginCommand(config, "check-container", plug)
	plcmd.RunE = func(cmd *cobra.Command, args []string) error {
		return run(cmd, args, "check-container", config, &preflight.ResultWriterFile{})
	}
	plcmd.Use = "container"
	return plcmd
}
