...
run:
	...
	ARGS={"$(ARGS)":""}
	go run cmd/preflight/main.go $(ARGS)

.PHONY: build
build:
	go build -o $(BINARY) -ldflags "-X github.com/redhat-openshift-ecosystem/preflight/version.commit=$(VERSION) -X github.com/redhat-openshift-ecosystem/preflight/version.version=$(RELEASE_TAG)" cmd/preflight/main.go
...
.PHONY: build-linux-$(1)
build-linux-$(1):
	GOOS=linux GOARCH=$(1) CGO_ENABLED=0 go build -o $(BINARY)-linux-$(1) -ldflags "-X github.com/redhat-openshift-ecosystem/preflight/version.commit=$(VERSION) \
				-X github.com/redhat-openshift-ecosystem/preflight/version.version=$(RELEASE_TAG)" cmd/preflight/main.go
endef
